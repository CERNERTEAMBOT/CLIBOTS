...
local function lock_group_super(msg, data, target)
  if not is_momod(msg) then
    return
  end
...
local function run(msg, matches)
   ...
   if matches[2] == 'edit' then
    savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked edit chars. in names")
    ...
    savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked edit Msg ")
    return lock_group_edit(msg, data, target)
   end
   ...
   if matches[2]:lower() == 'edit' then
    savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked edit ")
